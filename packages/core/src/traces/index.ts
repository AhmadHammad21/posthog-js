...
export class PostHogTraces {
  ...
  startSpan(name: string, options?: StartSpanOptions): Span {
    ...
    if (explicitParent && typeof explicitParent !== 'string' && !(explicitParent instanceof PostHogSpan)) {
      ...
      // header arrives twice, and W3C treats that as no inbound context. The
      // unusable parent is ignored: the span is parented to the active span if
      // one is active, and starts a new trace if not.
  ...
  // Retry delay after a failed flush: the first retry is at the base interval,
  // then doubling, capped at 30s — and never below an interval a host already
  // configured above the cap.
