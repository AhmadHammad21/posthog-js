...
export abstract class PostHogBackendClient extends PostHogCoreStateless implements IPostHog {
   ...
   * sessions.
   *
   * A server process has no ambient identity, so these come from the current
   * request context — populated by the Express/NestJS middleware from the
   * `X-POSTHOG-DISTINCT-ID` / `X-POSTHOG-SESSION-ID` tracing headers, or set
   * directly via `withContext`. Outside a request the keys are simply omitted.
   */
   ...
   * you — at return for a sync callback, at settle for an async one.
   *
  ...
  async _shutdown(shutdownTimeoutMs?: number): Promise<void> {
    ...
    if (this._traces) {
      ...
      // shutdown budget so an unresponsive transport can't hold shutdown past
      // the caller's deadline, then reset so a flush that lost the race can't
      // re-arm a timer after teardown.
      await raceWithTimeout(
